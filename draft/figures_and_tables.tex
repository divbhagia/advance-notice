...
% % \begin{landscape}
% % \begin{table}
% %     \input{\outdir/tab_sum_stats.tex}
% % \end{table}
% % \end{landscape}
...
\begin{figure}[t]\caption{Calibration of the Search Model}\label{fig_calibration}
  ...
  \includegraphics{./../output/fig_calib_offer}
  \subcaption{Offer Arrival Rate}
  \end{subfigure} 
  \begin{subfigure}{.475\textwidth}
  \includegraphics{./../output/fig_calib_search}
  \subcaption{Search Effort}
  \end{subfigure}
  \floatfoot{\textit{Notes}: The figure presents the search effort and offer arrival rate from the calibration of the search model, assuming no heterogeneity (dashed line) and assuming two types of workers (solid line). The search effort is averaged over two types of workers.}
  \end{figure}
...
\begin{subfigure}{.49\linewidth}
\raggedleft
\includegraphics{\outdir/search_model_fitA}
\subcaption{Observed Hazard}
\end{subfigure} \hfill
\begin{subfigure}{.49\linewidth}
\includegraphics{\outdir/search_model_fitB}
\subcaption{Structural Hazard}
\end{subfigure} 
\vspace{-0.75em}
\floatfoot{\textit{Note:} The figure displays the fit of the search model for the two calibration exercises described in the text. Panel A shows the observed exit rate in the data (solid-line) alongside the corresponding fitted values obtained from calibrating the search model without heterogeneity (dotted-line) and with two types of workers (dashed-line). Panel B displays the estimated structural hazard from the Mixed Hazard (MH) model (solid-line) and the fitted structural hazard from calibrating the search model with two types of workers (dashed-line).}
\end{figure}
...
\begin{center}
\sc{\Large Online Appendix} \\ \vspace{0.25cm} \sc{\Large Duration Dependence and Heterogeneity: \\ Learning from Early Notice of Layoff} \\ \vspace{0.5cm}
\setcounter{footnote}{1}
\renewcommand*{\thefootnote}{\fnsymbol{footnote}}
\textit{\large \sc{Div Bhagia}}\footnote{California State University, Fullerton; dbhagia@fullerton.edu}\\ 
\date{\large \today}
\end{center}
...
\begin{figure}[p]\caption{Industry and Occupation of the Lost Job}\label{fig_occ_ind_bal}
  ...
  \includegraphics[trim = 0.5in 0in 0in 0in]{./../output/fig_ind_ub.pdf}
  \caption{Industry, Unbalanced}
  \end{subfigure} 
  \begin{subfigure}{0.475\textwidth}
  \includegraphics[trim = 0.5in 0in 0in 0in]{./../output/fig_ind_bal.pdf}
  \caption{Industry, Balanced}
  \end{subfigure}
    \begin{subfigure}{0.51\textwidth}
  \includegraphics[trim = 0.5in 0in 0in 0in]{./../output/fig_occ_ub.pdf}
  \caption{Occupation, Unbalanced}
  \end{subfigure} 
  \begin{subfigure}{0.475\textwidth}
  \includegraphics[trim = 0.5in 0in 0in 0in]{./../output/fig_occ_bal.pdf}
...
& (1) & (2) & (3) & (4) \\
\midrule
\input{./../output/tab_reg_wagesA.tex} \addlinespace[2ex]
Controls   &  No & Yes  & No & Yes \\
Weights   & No  & No   & Yes & Yes \\
\midrule
\input{./../output/tab_reg_wagesB.tex}
\bottomrule
\end{tabularx}
\begin{tablenotes}
...
\includegraphics{./../output/fig_uiex_break.pdf}
...
\end{figure}
  ...
  % FIGURE: Alternative Bins 9 weeks: DATA
\begin{figure}[p]\caption{Survival and Exit Rates with Alternative Bins}\label{fig_altbins}
\begin{subfigure}{.475\textwidth}
\centering
\includegraphics{./../output/fig_4week_survival.pdf}
\subcaption{Survival Rate}
\end{subfigure}
\begin{subfigure}{.475\textwidth}
\centering
\includegraphics{./../output/fig_4week_hazard.pdf}
\subcaption{Exit Rate}
\end{subfigure}
\begin{subfigure}{.475\textwidth}
\includegraphics{./../output/fig_9week_survival.pdf}
\subcaption{Survival Rate}
\end{subfigure}
\begin{subfigure}{.475\textwidth}
\includegraphics{./../output/fig_9week_hazard.pdf}
\subcaption{Exit Rate}
\end{subfigure}
...
\centering 
\includegraphics{./../output/fig_robust_np}
\floatfoot{\textit{Note:} Figure presents estimates for the structural hazard from the Mixed Hazard model under alternative parametric assumptions. Error bars represent 90\% confidence intervals.}
\end{figure}
...
\begin{subfigure}{.475\textwidth}
\centering
\includegraphics{./../output/fig_hazard_unwtd.pdf}
\subcaption{Exit Rate}
\end{subfigure}
\begin{subfigure}{.475\textwidth}
\includegraphics{./../output/fig_robust_unwtd.pdf}
\subcaption{Estimated Structural Hazard}
\end{subfigure}
\floatfoot{\textit{Note:} The figure presents data and estimates for the unweighted analytical sample. Panel A presents the exit rate from the data separately for long and short-notice workers. The solid line in panel B shows the estimated structural hazard from the Mixed Hazard model, while the dotted line represents the average exit rate for short and long-notice workers in the data.}
\end{figure}

% FIGURE: Simulation: Average Estimate
\begin{figure}[h]\caption{Simulation: Average Estimate}\label{fig_simulation_average}
  \includegraphics{./../output/fig_sm_sim_avgpred.pdf}
  \floatfoot{\textit{Note:}  The solid line presents the average estimate from 1000 simulations of the search model. The dashed line presents the structural duration dependence $\E [h(d|\nu)]$ implied by the model. While the dotted line presents the observed structural duration dependence $\E [h(d|\nu)|D \geq d]$ implied by the model.}
  \end{figure}
...
\includegraphics{./../output/fig_sm_sim_dist1.pdf}
\subcaption{$\psi(1)$}
\end{subfigure} 
\begin{subfigure}{.4\textwidth}
\includegraphics{./../output/fig_sm_sim_dist2.pdf}
\subcaption{$\psi(2)$}
\end{subfigure} 
\begin{subfigure}{.4\textwidth} 
\includegraphics{./../output/fig_sm_sim_dist3.pdf}
\subcaption{$\psi(3)$}
\end{subfigure} 
\begin{subfigure}{.4\textwidth}
\includegraphics{./../output/fig_sm_sim_dist4.pdf}
\subcaption{$\psi(4)$}
\end{subfigure}
...
\makebox[\textwidth]{
\begin{subfigure}{.475\textwidth}
\includegraphics{./../output/fig_extgen_strhazA.pdf}
\subcaption{Residuals}
\end{subfigure} 
\begin{subfigure}{.475\textwidth}
\includegraphics{./../output/fig_extgen_strhazB.pdf}
\subcaption{10 lowest-residual estimates}
\end{subfigure} 
\floatfoot{\textit{Note:}  The figure presents results from the estimation of a more generalized Mixed Hazard model, where the structural hazard after the initial period for individuals with different lengths of notice is allowed to vary according to the parameter $\gamma$. Panel A presents the residuals from GMM estimation for different values of $\gamma$. Panel B presents the estimates of structural hazard for different values of $\gamma$.}
}
\end{figure} 
...
\makebox[\textwidth]{
\begin{subfigure}{.475\textwidth}
\includegraphics{./../output/fig_extgen_bothA.pdf}
\subcaption{Residuals}
\end{subfigure} \hspace{1em}
\begin{subfigure}{.475\textwidth}
\includegraphics{./../output/fig_extgen_bothB.pdf}
\subcaption{25 lowest-residual estimates}
\end{subfigure} 
\floatfoot{\textit{Note:}  The figure presents results from the estimation of a more generalized Mixed Hazard model. The mean of the heterogeneity distribution for individuals with different lengths of notice is allowed to vary according to the parameter $\kappa_1$. The structural hazard after the initial period for individuals with different lengths of notice is allowed to vary according to the parameter $\gamma$. Panel A presents the residuals from GMM estimation for different values of $\kappa_1$ and $\gamma$. Panel B presents the estimates of structural hazard for the case where $\kappa_1=0$ and $\gamma=1$ (solid line) and for the case when $\kappa_1$ and $\gamma$ take values that minimize the residual in Panel A (dashed line).} 
}
\end{figure}
